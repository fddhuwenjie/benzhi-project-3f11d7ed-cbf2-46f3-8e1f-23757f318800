...
func NewService(store Store, evidence EvidencePort, clock Clock, idGenerator func() string) *Service {
	return &Service{store: store, evidence: evidence, clock: clock, ids: idGenerator}
}
...
func (s *Service) Timeline(ctx context.Context, id string) ([]domain.AuditEvent, error) {
	...
	if err != nil {
		if errors.Is(err, context.Canceled) || errors.Is(err, context.DeadlineExceeded) || errors.Is(err, ErrAuditCorrupt) || errors.Is(err, ErrRevisionConflict) || errors.Is(err, ErrIdempotencyConflict) {
			return nil, err
		}
		return nil, fmt.Errorf("读取 timeline 失败: %v", err)
	}
	return events, nil
}
