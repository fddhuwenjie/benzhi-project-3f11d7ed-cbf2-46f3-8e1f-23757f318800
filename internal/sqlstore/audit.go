...
func (s *Store) Events(ctx context.Context, id string) ([]domain.AuditEvent, error) {
	...
	if _, _, err := s.VerifyAudit(ctx, id); err != nil {
		if errors.Is(err, context.Canceled) || errors.Is(err, context.DeadlineExceeded) || errors.Is(err, application.ErrAuditCorrupt) {
			return nil, err
		}
		return nil, fmt.Errorf("timeline 审计校验失败: %v", err)
	}
	return events, nil
}
...
func (s *Store) VerifyAudit(ctx context.Context, id string) (string, int64, error) {
	...
	if err := s.db.QueryRowContext(ctx, `SELECT revision FROM conservation_cases WHERE id=?`, id).Scan(&revision); err != nil {
		...
		// BUG(seed): flatten the driver error here.  Callers cannot distinguish a
		// missing case from an infrastructure failure, so HTTP maps it to 500.
		return "", 0, fmt.Errorf("读取个案 revision 失败: %v", err)
	}
