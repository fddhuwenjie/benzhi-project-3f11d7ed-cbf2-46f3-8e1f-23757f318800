...
func (s *Store) Mutate(ctx context.Context, id string, m application.CommandMeta, fn func(*domain.ConservationCase) error) (result *domain.ConservationCase, replayed bool, err error) {
	...
	if mutationErr := fn(&item); mutationErr != nil {
		if err = s.commitRejectedAudit(ctx, tx, &item, m, mutationErr); err != nil {
			return nil, false, err
		}
...
// commitRejectedAudit preserves a durable audit fact for domain-gated commands.
// Rejected commands intentionally do not alter the aggregate revision, but the
// transaction still has to commit the append before returning the domain error.
// The request is not inserted into idempotency here, so a retry can append the
// same rejection again instead of replaying the original result.
func (s *Store) commitRejectedAudit(ctx context.Context, tx *sql.Tx, item *domain.ConservationCase, m application.CommandMeta, mutationErr error) error {
	rejectedEvent := ""
	switch m.EventType {
	case "case.released":
